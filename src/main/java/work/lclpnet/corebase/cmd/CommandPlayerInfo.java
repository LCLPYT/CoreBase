...
public class CommandPlayerInfo extends CommandBase {
    ...
    private void info(CommandSource source, ServerPlayerEntity target) {
        try {
            ...
            if (target.connection.getNetworkManager().isLocalChannel())
                msg(source, "IP-ADDRESS", target.connection.getNetworkManager().getRemoteAddress().toString());
            else
                msg(source, "IP-ADDRESS", ((InetSocketAddress) target.connection.getNetworkManager().getRemoteAddress()).getHostString());
