...
public class CommandDie extends CommandBase {
    ...
    public int dieMany(CommandContext<CommandSource> ctx) throws CommandSyntaxException {
        ...
        if (died.isEmpty())
            ctx.getSource().sendErrorMessage(CoreBase.TEXT.message("None of the entites you supplied are alive (have health) and therefore can't be killed.", MessageType.ERROR));
        else
            ctx.getSource().sendFeedback(CoreBase.TEXT.complexMessage("You killed %s.", TextFormatting.GREEN, new Substitute(EntitySelector.joinNames(died).getString(), TextFormatting.YELLOW)), false);
